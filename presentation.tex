...
\begin{frame}[fragile]{Soll-Analyse}
  ...
  \begin{itemize}
    \item Paketgröße und Anzahl automatisch erfassen
          \note<1->[item]{Automatische Erfassung}
          \pause
    \item Erfüllung der Anforderungen durch Verpackungsgesetz
          \note<2->[item]{Helfen bei Anforderungen durch Verpackungsgesetz}
          \pause
...
\note[itemize]
{
  ...
  \item{3 verschiedene Sensor-Typen getestet}
}
...
\note[itemize]
{
  ...
  \item{Etwa 30k Euro guenstiger in Eigenentwicklung}
...
\begin{frame}[fragile]{Problemstellung Rollen und YOLOv7}
  \begin{minipage}[t]{0.49\textwidth}
    \begin{figure}[htpb]
      \centering
      \includegraphics[angle=90,origin=c,width=1\textwidth]{pics/ShippingBox.jpg}
    \end{figure}
  ...
  \begin{minipage}[t]{0.49\textwidth}
    ...
    \begin{itemize}
      ...
      \item Erkennung von Objekten durch Semantische Segmentierung
            \note<3->[item]{Suche nach Bedeutung in Bildern, pixelweise}
            \pause
      \item Leichtes Training durch Bag of Freebies und Transfer Learning
            \note<4->[item]{Bag of Freebies: Methoden, die die Performance eines Modells erhöhen, ohne die Trainingszeit zu verlängern}
    \end{itemize}
  \end{minipage}
\end{frame}
...
\note[itemize]
{
  ...
  \item{Traingsbefehlt hier zu sehen}
...
\begin{frame}[fragile]{Lessons Learned und Ausblick}
  ...
  \begin{itemize}
    ...
    \item Industriekamera $>$ Webcam $\rightarrow$ KEYENCE-Scanner
          \note<3->[item]{Budget besser setzen}
  \end{itemize}
